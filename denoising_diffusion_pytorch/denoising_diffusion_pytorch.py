...
class Trainer:
    ...
    def save(self, milestone):
        ...
        path_best_model = str(self.results_folder / f'model-{milestone}.pt')
        torch.save(data, path_best_model)
                ...
                if accelerator.is_main_process:
                    ...
                    if self.step != 0 and divisible_by(self.step, self.save_and_sample_every):
                        ...
                        if self.calculate_fid:
                            ...
                            self.run.log({"FID Score": fid_score})
                        if self.save_best_and_latest_only:
                            if self.best_fid > fid_score:
                                self.best_fid = fid_score
                                path_best_model = self.save("best")
                            path_last_model= self.save("latest")
                        else:
                            path_last_model = self.save(milestone)

                pbar.update(1)
            self.run.log(log_dict)
        accelerator.print('training complete')
        self.run.log_model(path=path_best_model, name=f'{self.run_name}-best-model')
        self.run.log_model(path=path_last_model, name=f'{self.run_name}-last-model')